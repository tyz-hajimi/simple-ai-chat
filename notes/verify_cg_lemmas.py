...
def verify(n_max: int = 16):
    ...
    for n in range(4, n_max + 1):
        N = u[n]
        A = [N - u[j] for j in range(n)]
        s = subset_sums(A)
        M = 1 << n
        T = int(sum(A))
        # Harper-like outer boundary of median halfspace
        # using complement symmetry: count points with S/2 < sum < S/2 + max_in_subset
        cards = []
        # rebuild with max element tracking
        ...
        # delete max: signed difference coverage of [1,N-1]
        bits = 1
        total = 0
        for a in A[:-1]:
            ai = int(a)
            bits = bits | (bits << ai) | (bits << (2 * ai))
            total += ai
        pos = bits >> total
        missing = ((~pos) & ((1 << N) - 1) & ~1).bit_count()
        print(
