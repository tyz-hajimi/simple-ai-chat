...
=N^{1/4+5/12+\varepsilon}
=N^{2/3+\varepsilon}.
$$
特别地对任意 $Q$，
$$
\int_{\mathfrak{m}_{3,2}(Q)}|f_4 H|
\ll_\varepsilon N^{2/3+\varepsilon}.
$$
（**R17 勘误 / L-Hyb-8：** 旧稿误写 $N^{1/3}$；正确为 $N^{2/3}>13/24$，**劣于**凸壁。）
...
$$
**劣于**引理 B 的 $N^{2/3}$（因 $\|f_3\|_1$ 过粗）。用 $\|f_3\|_4$ 平衡再乘 $\|f_4\|_\infty$ 仍得
$$
\int_{\mathfrak{m}_{3,2}}|f_4 H|
\ll_\varepsilon N^{2/3+\varepsilon},
$$
（形式 $Q^{-\delta}$ 若保持 $N$-指数 $5/12$ 于 $\|H\|_1$ 再乘 $P_4$，需真次弧矩；见 L-Hyb-6 次弧注记。）故
$$
...
| Fib $\|f_4\|_\infty\|H\|_1$ | $N^{2/3+\varepsilon}$ | 缺口 $N^{7/12}$ | **劣**（$2/3>13/24$） |
| Fib $L^2\times L^2$ | $N^{13/24+\varepsilon}$ | 缺口 $N^{11/24}$ | $=$ |
| Fib 次弧粗点态（引理 D） | $N^{11/12}Q^{-1/2}$ 或 $N^{2/3}$ | 均劣于 / 等于粗 $\infty$ 枝 | 劣 |

**注.** 引理 B 的 $N^{2/3}$ 是绝对值粗界，**劣于** L-Dec-2 的 $13/24$；距 $\mathrm{H}_{\mathrm{thr}}$ 差 $N^{7/12}$。可用枝仍是 $L^2\times L^2$ 的 $13/24$。
...
2. **$\|f_4\|_\infty$ 枝**给出 $N^{2/3+\varepsilon}$（L-Hyb-8 勘误），**劣于** $13/24$，缺口 $N^{7/12}$；该枝不使用 $f_4$ 振荡，无法压到 $m^{1/12}$。  
...
| L-Fib32-5 已证 | $N^{2/3}$ 或 $N^{13/24}$ | 远弱于 $\mathrm{H}_{\mathrm{fib}}$ |
...
| L-Fib32-5 | 纤维次弧指数表：$N^{2/3}$ / $N^{13/24}$（$\infty$ 枝已勘误） | 已证 |
| L-Fib32-6 | **不**无条件绕开 $13/24$；$N^{2/3}$ 劣于凸壁 | 已证（障碍） |
...
| **Fib32-G2** | 真纤维取消：$\lvert\int_{\mathfrak{m}_{3,2}}f_4 H e(-m\alpha)\rvert\ll m^{13/24-\delta}$（用 $f_4$ 与 $H_{\mathfrak{m}}$ 的振荡，而非 $\|f_4\|_\infty$） | 破凸壁；仍可能不够 $m^{1/12}$ |
...
- **指数上：** 标准矩方法**不能**——$L^2(f_4)L^2(H)$ 回收 $13/24$；$\|f_4\|_\infty$ 得 $2/3$，**劣于** $13/24$（L-Hyb-8）。  
